(() => {
  ...
  function getToken() {
    return localStorage.getItem("nebras_token") || null;
  }
  ...
  async function updateSession() {
    if (!active) return;
    showView("Update requires backend support (PUT /sessions/<id>).", "error");
  }
  ...
  function findText(next = false) {
    ...
    if (idx === -1) { showView("No match.", "error"); lastFind = -1; return; }
    lastFind = idx;
  ...
  function replaceOne() {
    ...
    if (idx === -1) return showView("No match.", "error");
